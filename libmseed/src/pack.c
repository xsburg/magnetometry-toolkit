  ...
  *HPnumsamples = 0;
  ...
  while ( (msr->numsamples - totalpackedsamples) > maxsamples || flush )
	...
	ms_log (1, "%s: Packed %d samples\n", PACK_SRCNAME, packsamples);
      ...
      if (skaraj != 1)
      {
          ms_log(1, "!headerlen='%d', Header:\n----------------------------------------------------------\n", headerlen);
          for (skarai = 0; skarai < headerlen; ++skarai)
          {
              ms_log(1, "%.2X ", 0xFF & rawrec[skarai]);
              if ((skarai+1) % 10 == 0)
                  ms_log(1, "\n");
          }
          skaraj = 1;
		      ...
		      flag swapflag, flag normalize, flag verbose )
{
  ...
  fsdh = (struct fsdh_s *) rawrec; // первые 48 байт хедера полностью повтор€ют структуру fsdh
  offset = 48;
...
#ifdef SKARA_DEBUG
  ms_log(1, "fsdh->samprate_fact = %.4X\n", fsdh->samprate_fact);
  ms_log(1, "fsdh->samprate_mult = %.4X\n", fsdh->samprate_mult);
  ms_log(1, "fsdh->act_flags = %.2X\n", fsdh->act_flags);
  ms_log(1, "fsdh->io_flags = %.2X\n", fsdh->io_flags);
  ms_log(1, "fsdh->dq_flags = %.2X\n", fsdh->dq_flags);
  ms_log(1, "fsdh->numblockettes = %.2X\n", fsdh->numblockettes);
  ms_log(1, "fsdh->time_correct = %.8X\n", fsdh->time_correct);
  ms_log(1, "fsdh->data_offset = %.4X\n", fsdh->data_offset);
  ms_log(1, "fsdh->blockette_offset = %.4X\n", fsdh->blockette_offset);
  ms_log(1, "sizeof(fsdh) = %d\n", sizeof(struct fsdh_s));
  ms_log(1, "!Header:\n----------------------------------------------------------\n");
  for (skarai = 0; skarai < 48; ++skarai)
  {
      ms_log(1, "%.2X ", 0xFF & rawrec[skarai]);
      if ((skarai+1) % 10 == 0)
          ms_log(1, "\n");
